...
clean_data <- try(function(x, func_path, lon_col, lat_col, time_col, rowid_col, 
                           dop_col = NULL, epsg, landuse = NULL, alpha, 
                           delta_speed, delta_time, epsilon, zeta, eta, theta){
...
# on pc
source("C:/Users/A02362279/Box/Avgar Lab on WILD/Code/Functions/
       GPS_datacleaning_funs.R")
  ...
  if(nrow(trk) <= 1){
    # 1) Check if the track has more than 1 record ----
    # if not, add a flag
    clean <- trk %>%
      dplyr::select(x_, y_, t_, row_id) %>%
      mutate(clean_why = "Only 1 record")
  } 
  # if so, continue
  else{
    # 2) Remove fixes influenced by the capture ----
...
# landcover raster
landcover <- raster("../../../Avgar Lab on WILD/UtahEnvironmentalCovariates/
                    Landcover/ut_landcover_utm.tif")
landcover
plot(landcover)
...
# on mac
# elk <- readRDS("~/Box/MSc work/elk_data_cleaning/
# 20220216_elk_data_BigQuery.rds")

# on desktop
elk_dat <- readRDS("C:/Users/A02362279/Box/MSc work/Elk_Cleaning_new/
                   20220216_elk_data_BigQuery.rds")

# Changing column names, removing unnecessary columns, and adding row id
elk <- elk_dat %>%
...
write_rds(clean, "clean_flagged_data.rds")
