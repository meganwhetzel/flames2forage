...
nd <- pred_data(scale_df, # add in all ndvi and cover variables
       ...
       #         NDVI.delta = c(0.2, 0.4, 0.7),
       #         NDVI.norm.curr = c(0.15, 0.49, 0.85),
       #         cover.shrub = 20,
       #         cover.herbaceous = 50,
       #         cover.tree = 30,
...
ggsave("9-panel_fig_reference_20230327.tiff", width = 180, height = 180, units = "mm",
...
# How to determine how long elk were tracked
# 
# time <- HMM_prepared_dat_20230113 %>% 
#   group_by(ID) %>% 
#   summarize(tracked = max(dt) - min(dt))
# 
# time2 <- complete_fire_data_20230115 %>% 
#   group_by(animalid) %>% 
#   summarize(tracked = max(dt) - min(dt))
