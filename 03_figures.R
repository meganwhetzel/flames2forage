...
dir.create("out/fig", showWarnings = FALSE)
...
# Load fitted models ----
gam_fire <- readRDS("out/gam_fire.rds")
gam_combo <- readRDS("out/gam_combo.rds")
...
# ... Veg categories and burn severity (Fig. 4) ----
# Create a 9-panel figure, which crosses all the veg categories and the fire
# severity categories
...
plot9 <- plot.data %>%
  ...
  geom_line(aes(y = predUB, years), linetype = "longdash", linewidth = 0.7) +
...
ggsave("out/fig/9-panel_fig_reference_black.tiff", plot = plot9,
       width = 180, height = 180, units = "mm",
       dpi = 600, compression = "lzw")

# ... temporal patterns (Fig. 3) ----
...
ggsave("out/fig/temporal_pattern_update.tiff", plot = temp_plot,
...
ggsave("out/fig/betas.tiff", plot = beta_plot, 
